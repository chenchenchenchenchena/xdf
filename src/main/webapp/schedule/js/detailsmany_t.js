        ...
        for (var i = 0; i < regionindex.length; i++) {
            ...
            for (var k = 0; k < stuall.length; k++) {
                ...
                if (stuall[k].StudentName.length > 3) {
                    $('.studentList ul').append('<li class="swiper-slide" style="font-size:.4' +'rem;">' +
                        '<a href="studentInfo.html?remark=1&studentNo=' + stuall[k].StudentCode + '&schoolId='+BeginDate[timeindex].SchoolId+'&tCode=1&studentName=' + stuall[k].StudentName + '">' +
                        '<span class="name-icon">' + stuall[k].StudentName.substring(2, stuall[k].StudentName.length) + '</span></a>' +
                        '<p style="font-size:.3rem;">' + stuall[k].StudentName + '</p>' +
                        '<i class="to-tel" tel="' + mobile + '" stuCode="' + stuall[k].StudentCode + '" ></i>' +
                        '<a class="to-learn" href="../learningSituation/reportstu_t.html?studentNo=' + stuall[k].StudentCode + '&tCode=1&studentName=' + stuall[k].StudentName + '">查看学情</a></li>');
                } else {
                    $('.studentList ul').append('<li class="swiper-slide">' +
                        '<a href="studentInfo.html?remark=1&studentNo=' + stuall[k].StudentCode + '&schoolId='+BeginDate[timeindex].SchoolId+'&tCode=1&studentName=' + stuall[k].StudentName + '">' +
                        '<span class="name-icon">' + stuall[k].StudentName.substring(stuall[k].StudentName.length - 2, stuall[k].StudentName.length) + '</span></a>' +
                        '<p >' + stuall[k].StudentName + '</p>' +
                        '<i class="to-tel" tel="' + mobile + '" stuCode="' + stuall[k].StudentCode + '" ></i>' +
                        '<a class="to-learn" href="../learningSituation/reportstu_t.html?studentNo=' + stuall[k].StudentCode + '&tCode=1&studentName=' + stuall[k].StudentName + '">查看学情</a></li>')
                }
            }
