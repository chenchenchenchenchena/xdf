...
function stuc(e){
	...
	$('.numb_log').click(function(){
		var stumore  = {'StudentCode':$('.stunum').val(),'wechatId':sessionStorage.openid,'nickName':encodeURIComponent(sessionStorage.nickname),'headImg': sessionStorage.headimgurl}
...
$('.tel_log').click(function(){
	var stuname = {'name':$('.stname').val(),'mobile':$('.phoneNumber').val(),'wechatId':sessionStorage.openid,'nickName':encodeURIComponent(sessionStorage.nickname),'headImg': sessionStorage.headimgurl}
	// var stuname = {};
	// var name = $('.stname').val();
...
$(document).on('click','.Relation',function(){
	if($(this).html()=='确认关联'){
		...
		var stumore = {'StudentCode':stunum,'wechatId':sessionStorage.openid,'Mobile':sessionStorage.stuTel,'nickName':encodeURIComponent(sessionStorage.nickname),'headImg': sessionStorage.headimgurl}
		ajax_S(url.s_bind,stumore,telbind)
		// $(this).html('解除关联')
	}else{
		var stunum  = $('.stu_num').eq($(this).parent().index()-1).text()
        var stumore = {'StudentCode':stunum,'wechatId':sessionStorage.openid,'nickName':encodeURIComponent(sessionStorage.nickname),'headImg': sessionStorage.headimgurl}
